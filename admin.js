...
function ajStatus(id, texto, cor) {
  ...
  if (el) { el.style.color = cor || '#94A4CC'; el.textContent = texto || ''; }
}
...
function atualizarPreviewCores() {
  ...
  p.innerHTML = '<div style="background:' + pal.cartao + ';border:1px solid ' + pal.borda + ';border-radius:8px;padding:14px;">' +
    '<div style="font:600 18px Georgia,serif;color:' + pal.texto + '">' + escPlano(BARBEARIA.marca.linha1 || 'Sua') + ' <em style="color:' + pal.destaque + '">' + escPlano(BARBEARIA.marca.linha2 || 'Barbearia') + '</em></div>' +
...
function renderFontesAjuste() {
  ...
  const t1 = BARBEARIA.marca.linha1 || 'Sua', t2 = BARBEARIA.marca.linha2 || 'Barbearia';
  el.innerHTML = FONTES_TOPO.map(f =>
    '<button type="button" class="aj-fonte' + (f.id === _fonteEdit ? ' sel' : '') + '" onclick="escolherFonte(\'' + f.id + '\')" aria-pressed="' + (f.id === _fonteEdit) + '">' +
      ...
      '<span style="color:#EBC531;' + estiloLinhaFonte(f.l2, 0.42) + 'margin-top:' + Math.round(parseFloat(f.l2.mt || 0) * 0.42) + 'px;">' + escPlano(t2) + '</span>' +
...
async function salvarIdentidade() {
  ...
  if (!nome || !linha1) { ajStatus('aj-identidade-status', 'Informe o nome da barbearia e a 1ª linha do topo do site.', '#e05555'); return; }
...
function renderPlanosEditor() {
  ...
  if (!_planosEdit.length) { el.innerHTML = '<p style="color:#5E6E9E;font-size:13px;font-family:Roboto,sans-serif;">Nenhum plano cadastrado. Clique em “Adicionar plano”.</p>'; return; }
...
function renderDisponibilidade() {
  ...
  const cardIndisp = (h, motivo) => '<div title="' + escPlano(motivo) + '" style="background:#0C1838;border:1px solid #16295C;border-radius:6px;padding:8px 6px;text-align:center;">' +
  ...
  grid.innerHTML = slots.map(h => {
    ...
    if (cobre.length) {
      ...
      return '<div title="' + tip + '" style="background:#3a2c08;border:1px solid #EBC531;border-radius:6px;padding:8px 6px;text-align:center;' + (comeca.length ? '' : 'opacity:.75;') + '">' +
